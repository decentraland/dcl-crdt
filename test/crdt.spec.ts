...
describe('CRDT protocol', () => {
  ...
  ;[true, false].forEach((delay) => {
    ...
    it(`${msg}should store the message A in all the clients`, async () => {
      ...
      await compare()
      expect(clientA.getState().get(key1).get(key2).data).toBe(messageA.data)
    })
    ...
    it(`${msg}should decline message A if both messages are sent at the same time and data B > data A`, async () => {
      ...
      await compare()
      expect(
        compareData(clientA.getState().get(key1).get(key2).data, messageB.data)
      ).toBe(true)
    })
    ...
    it(`${msg}should store both keys`, async () => {
      ...
      await compare()
      expect(clientA.getState().get(key1b).get(key2b).data).toBe(messageA2.data)
    })
    ...
    it(`${msg}should store both keys, even if we send the messages in diff order z > a`, async () => {
      ...
      await Promise.all([p1, p2])
      await compare()
      expect(clientA.getState().get(key1b).get(key2b).data).toBe(messageB2.data)
    })
